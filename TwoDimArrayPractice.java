    ...
    // Write your code here:
	   if (row < 0 || row >= intArray.length) {
	    ...
	    for (int column = 0; column < intArray[row].length; column++) {
	        ...
	        animate(row, column);  // Animate setting each element
	    }
    ...
    // Write your code here:
	   int min = intArray[0][column];
	    for (int row = 1; row < intArray.length; row++) {
	        ...
	        animate(row, column, min); // animates minimum search for visual feedback
	    }
	    return min;
   }
	   ...
	   int count = 0;
	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            ...
	            animate(row, column, count); // animates count process for visual feedback
	        }
	    }
	    return count;
   }
